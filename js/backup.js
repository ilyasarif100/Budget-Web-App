 ...
 * @returns {Promise<Object>} Complete data export object with version, exportDate, data, and metadata
 ...
 * @param {string} [filename=null] - Optional filename (default: budget-backup-YYYY-MM-DD.json)
...
async function exportAllDataToFile() {
  try {
    ...
    if (success) {
      ...
      console.log(
        `Exported ${data.metadata.transactionCount} transactions, ${data.metadata.accountCount} accounts, ${data.metadata.categoryCount} categories`
      );
    }
