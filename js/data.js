...
const accountsMap = new Map(); // O(1) account lookups
const transactionsMap = new Map(); // O(1) transaction lookups by ID
const transactionsByAccountMap = new Map(); // O(1) transactions by accountId
const transactionsByPlaidIdMap = new Map(); // O(1) transactions by plaidTransactionId
const transactionsByCategoryMap = new Map(); // O(1) transactions by category
const transactionsByDateMap = new Map(); // O(1) transactions by date (YYYY-MM-DD)
...
const dirtyTransactions = new Set(); // Track which transactions changed
const dirtyAccounts = new Set(); // Track which accounts changed
const dirtyCategories = new Set(); // Track which categories changed
let forceFullSave = false; // Flag to force full save when needed

// Build accounts map for O(1) lookups
/**
 * Builds a Map structure for O(1) account lookups by ID
 * @returns {void}
 */
...
/**
 * Builds Map structures for O(1) transaction lookups by various keys
 * Creates maps for: ID, accountId, plaidTransactionId, category, and date
 * @returns {void}
 */
  ...
  try {
    ...
    if (forceAll || forceFullSave || dirtyTransactions.size > 0) {
      const transactionsToSave =
        forceAll || forceFullSave
          ? transactions
          : Array.from(dirtyTransactions)
    ...
    if (forceAll || forceFullSave || dirtyCategories.size > 0) {
      const categoriesToSave =
        forceAll || forceFullSave ? categories : categories.filter(c => dirtyCategories.has(c.id));
...
async function clearAllData() {
  ...
  try {
    ...
    if (typeof editingTransactionId !== 'undefined') {
