...
const plaidItemIds = new Map(); // Map of account_id -> { item_id, cursor } (NO ACCESS TOKENS)

// Lazy load Plaid SDK
let plaidSDKLoaded = false;
/**
 * Lazy loads the Plaid Link SDK from CDN
 * @returns {Promise<void>} Resolves when SDK is loaded and available
 * @throws {Error} If SDK fails to load within 10 seconds
 */
...
/**
 * Initializes Plaid Link for connecting bank accounts
 * @returns {Promise<void>} Resolves when Plaid Link is initialized
...
async function handlePlaidSuccess(publicToken, metadata) {
  try {
    ...
    const tokenResponse = await authenticatedFetch(
      `${window.CONFIG.API_BASE_URL}/item/public_token/exchange`,
      {
        method: 'POST',
        body: JSON.stringify({
    ...
    // NO RETRY - if server is down, fail fast with clear error
    const accountsResponse = await authenticatedFetch(
      `${window.CONFIG.API_BASE_URL}/accounts/get`,
      {
        method: 'POST',
        body: JSON.stringify({
          item_id: itemId,
    ...
    for (const plaidAccount of accountsData.accounts) {
      ...
      if (!existingAccount) {
        ...
        existingAccount.initialBalance =
          plaidAccount.balances.current || existingAccount.initialBalance;
    ...
    if (newlyAddedAccountIds.length > 0) {
      showToast(
        `${newlyAddedAccountIds.length} account(s) added and synced successfully!`,
        'success'
      );
    } else {
