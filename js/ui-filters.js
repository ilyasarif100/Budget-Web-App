...
function populateSyncAccountsModal() {
    ...
    const plaidAccounts = accounts.filter(acc => acc.plaidItemId && (typeof plaidItemIds !== 'undefined' ? plaidItemIds.has(acc.id) : false));
    ...
    plaidAccounts.forEach(acc => {
        ...
        const lastSyncTime = typeof getAccountSyncTime !== 'undefined' ? getAccountSyncTime(acc.id) : null;
        const syncStatusColor = typeof getSyncStatusColor !== 'undefined' ? getSyncStatusColor(lastSyncTime) : 'var(--text-secondary)';
...
async function confirmSyncSelectedAccounts() {
    ...
    accounts.forEach(acc => {
        if (acc.plaidItemId && (typeof plaidItemIds !== 'undefined' ? plaidItemIds.has(acc.id) : false)) {
