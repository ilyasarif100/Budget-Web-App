...
function updateTotalBalance() {
    ...
    totalBalanceEl.textContent = typeof formatCurrency !== 'undefined' ? formatCurrency(total) : `$${total.toFixed(2)}`;
}
...
function updateBalanceDisplay() {
    ...
    accountsSumDisplay.textContent = typeof formatCurrency !== 'undefined' ? formatCurrency(total) : `$${total.toFixed(2)}`;
}
...
function updateTotalSpent() {
    ...
    const total = filteredTransactions
        .filter(t => {
            ...
            if (typeof includedSpentAccountIds !== 'undefined' && includedSpentAccountIds.size > 0 && !includedSpentAccountIds.has(t.accountId)) return false;
            
            // Check category filter
            if (t.category) {
                // Has category - check if category is included
                const category = categories.find(c => c.name === t.category);
                if (category && typeof includedSpentCategoryIds !== 'undefined' && includedSpentCategoryIds.size > 0 && !includedSpentCategoryIds.has(category.id)) {
                    return false;
                }
            } else {
                // No category (exempt) - check if exempt is included
                if (typeof includeExemptInSpent !== 'undefined' && !includeExemptInSpent) return false;
            }
    ...
    totalSpentEl.textContent = typeof formatCurrency !== 'undefined' ? formatCurrency(total) : `$${total.toFixed(2)}`;
}
...
function updateSpentDisplay() {
    ...
    if (spentSumDisplay) {
        spentSumDisplay.textContent = typeof formatCurrency !== 'undefined' ? formatCurrency(total) : `$${total.toFixed(2)}`;
    }
}
