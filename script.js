...
async function syncAllTransactionsInternal(accountIdsToSync = null) {
  ...
  async function syncSingleAccount(accountId) {
    ...
    try {
      const account =
        typeof accountsMap !== 'undefined' && accountsMap
      ...
      while (hasMore && pageCount < maxPages) {
        ...
        if (!response.ok) {
          ...
          if (!errorText.includes('Cannot connect to server')) {
            console.error(
              `Failed to sync transactions for account ${accountId}: ${response.status} ${errorText}`
            );
          }
          break;
        }
        ...
        for (const plaidTransaction of syncData.transactions) {
          ...
          const existingTransaction =
            typeof transactionsByPlaidIdMap !== 'undefined' && transactionsByPlaidIdMap
          ...
          if (!existingTransaction && accountMatches) {
            ...
            const newTransaction = {
              ...
              accountId: accountId,
              accountType:
                account.type === 'credit' || account.type === 'loan' ? 'credit' : 'debit',
        ...
        for (const plaidTransaction of syncData.modified) {
          ...
          if (existingTransaction) {
            ...
            existingTransaction.category = plaidTransaction.category
      ...
      try {
        const accountsResponse = await retryWithBackoff(
          ...
          2,
          500
        ); // Fewer retries for balance update (less critical)

        if (accountsResponse.ok) {
          const accountsData = await accountsResponse.json();
          const plaidAccount = accountsData.accounts.find(
  ...
  for (let i = 0; i < accountsToSync.length; i += BATCH_SIZE) {
    ...
    const batchResults = await Promise.all(
      batch.map(accountId =>
        syncSingleAccount(accountId).catch(err => {
  ...
  if (syncedCount > 0 || modifiedCount > 0 || removedCount > 0) {
    ...
    let message = '';
    if (syncedCount > 0) {
      message += `Synced ${syncedCount} new transaction(s)`;
    }
    if (modifiedCount > 0) {
      message += `${message ? ', ' : ''}Updated ${modifiedCount} transaction(s)`;
    }
    if (removedCount > 0) {
      message += `${message ? ', ' : ''}Removed ${removedCount} transaction(s)`;
    }
...
const sortConfig = { column: 'date', direction: 'desc' };
let editingCategoryId = null;
let editingTransactionId = null;
let editingAccountId = null;
let deletingTransactionId = null; // Track which transaction is being deleted
const dateFilter = { type: 'current-month', startDate: null, endDate: null };
...
async function deleteCategory(id) {
  ...
  const hasTransactions = transactions.some(
    t => t.category === category.name && t.status !== 'removed'
  );

  if (hasTransactions) {
    if (
      !confirm(
        `This category has transactions. Are you sure you want to delete "${category.name}"? Transactions will be set to Exempt.`
      )
    ) {
      return;
    }
...
async function deleteAccount(id) {
  ...
  const accountTransactions =
    typeof transactionsByAccountMap !== 'undefined' && transactionsByAccountMap
  ...
  if (index !== -1) {
    ...
    if (deletedTransactionCount > 0) {
      const deletePromises = transactionsToDelete.map(t =>
        deleteFromStore(STORES.TRANSACTIONS, t.id)
      );
...
function validateAccountForm() {
  ...
  const existingAccount = accounts.find(
    a => a.name.toLowerCase() === name.toLowerCase() && a.id !== editingAccountId
  );
  if (existingAccount) {
...
function validateCategoryForm() {
  try {
    ...
    const existingCategory = categories.find(
      c => c.name.toLowerCase() === nameValidation.value.toLowerCase() && c.id !== editingCategoryId
    );
    if (existingCategory) {
...
function initializeIncludedAccounts() {
  ...
  if (saved) {
    try {
      ...
      includedAccountIds = new Set(
        [...includedAccountIds].filter(id => accounts.some(acc => acc.id === id))
      );
    } catch (e) {
...
function initializeIncludedSpent() {
  ...
  if (savedCategories) {
    try {
      ...
      includedSpentCategoryIds = new Set(
        [...includedSpentCategoryIds].filter(id => categories.some(cat => cat.id === id))
      );
    } catch (e) {
  ...
  if (savedAccounts) {
    try {
      ...
      // Only include accounts that still exist
      includedSpentAccountIds = new Set(
        [...includedSpentAccountIds].filter(id => accounts.some(acc => acc.id === id))
      );
    } catch (e) {
...
function editTotalSpent() {
  ...
  if (!categoriesList || !accountsList) {
    return;
  }
  ...
  if (categories.length === 0) {
    categoriesList.innerHTML =
      '<p style="color: var(--text-secondary); padding: 1rem; text-align: center;">No categories available</p>';
  } else {
    categories.forEach(cat => {
      ...
      checkbox.checked =
        includedSpentCategoryIds.size === 0 || includedSpentCategoryIds.has(cat.id);
      ...
      label.style.cssText =
        'flex: 1; cursor: pointer; font-weight: 500; color: var(--text-primary);';
  ...
  if (accounts.length === 0) {
    accountsList.innerHTML =
      '<p style="color: var(--text-secondary); padding: 1rem; text-align: center;">No accounts available</p>';
  } else {
    accounts.forEach(acc => {
...
function renderCategoriesExpanded() {
  ...
  if (!isMonthly) {
    ...
    messageEl.style.cssText =
      'padding: 1rem; text-align: center; color: var(--text-secondary); font-size: 0.875rem;';
    messageEl.innerHTML = `
  ...
  if (containerHeight === 0 || containerHeight < 100) {
    ...
    if (parent && parent.clientHeight > 200) {
      ...
      if (estimatedHeight > 100) {
        ...
        virtualScrollState.startIndex = Math.max(
          0,
        ...
        virtualScrollState.endIndex = Math.min(
          ...
          virtualScrollState.startIndex +
            virtualScrollState.visibleCount +
            virtualScrollState.buffer * 2
        );
        return;
      }
    }
...
function renderTransactions() {
  ...
  const useVirtualScrolling =
    virtualScrollState.enabled && filteredTransactions.length > virtualScrollState.threshold;
    ...
    rafId = requestAnimationFrame(() => {
      ...
      if (Math.abs(scrollTop - lastScrollTop) > 5 || scrollTop === 0) {
        ...
        if (
          ...
          filteredTransactions.length > virtualScrollState.threshold
        ) {
          renderTransactions();
        }
      }
...
function editTransaction(id) {
  ...
  if (accountSelect) {
    ...
    if (
      ...
      (accountSelect.options.length === 1 && accountSelect.options[0].value === '')
    ) {
      if (window.populateTransactionAccountSelect) {
        window.populateTransactionAccountSelect();
      }
...
function setupEventListeners() {
  ...
  document
    ...
    .addEventListener('change', debouncedFilterTransactions);
  ...
  connectPlaidBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      const showError =
        window.showPlaidError || (typeof showPlaidError !== 'undefined' ? showPlaidError : null);
      if (showError) {
